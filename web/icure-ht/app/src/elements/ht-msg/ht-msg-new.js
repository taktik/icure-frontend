...
`;
  ...
  _patientFilterChanged(patientSearchValue) {
      ...
      setTimeout(() => {
          ...
          this.api.getRowsUsingPagination((key, docId, pageSize) =>
              ...
              .catch(e=>{console.log("ERROR with _patientFilterChanged: ", e);})
  ...
  _fileToBeUploadedChanged(filesToBeUploaded) {
      ...
      return !haveFilesToUploaded ? false : setTimeout(() => {
          ...
          Promise.all(_.filter(filesToBeUploaded, singleFileToBeUploaded => !singleFileToBeUploaded.attached).map(singleFileToBeUploaded => this.api.document().newInstance(this.user, null, { documentType: 'result', mainUti: this.api.document().uti(singleFileToBeUploaded.type), name: singleFileToBeUploaded.name })
  ...
  _fileGotSuccessFullyUploaded(e){
      ...
      if (!_.size(currentSubContacts)) { currentSubContacts = { status: 64, services: [] }; this.push("currentContact.subContacts", currentSubContacts); }

      const svc = this.api.contact().service().newInstance(this.user, { content: _.fromPairs([[this.language, { documentId: uploadedFileDocumentId, stringValue: _.trim(_.get(uploadedFileObject,"name","document.txt")) }]]), label: 'document' });
      ...
      setTimeout(() => {
          ...
          let searchProm = recipientType.id === 'CBE' ?
              Promise.all([
                  ...
                  this.api.fhc().Addressbookcontroller().searchHcpUsingGET(this.api.keystoreId, this.api.tokenId, this.credentials.ehpassword, _.trim(recipientFilter) + '*').then(searchResults=>_.chain(searchResults).filter(i=>!!_.get(i,'nihii',false) || !!_.get(i,'ssin',false) || !!_.get(i,'ehp',false) || !!_.get(i,'cbe',false)).uniqBy('nihii').value()),
                  this.api.fhc().Addressbookcontroller().searchOrgUsingGET(this.api.keystoreId, this.api.tokenId, this.credentials.ehpassword, _.trim(recipientFilter) + '*').then(searchResults=>_.chain(searchResults).filter(i=>!!_.get(i,'nihii',false) || !!_.get(i,'ssin',false) || !!_.get(i,'ehp',false) || !!_.get(i,'cbe',false)).uniqBy('ehp').value())
              ]) :
              ...
              (recipientType.id === 'NIHII-HOSPITAL' || recipientType.id === 'NIHII-PHARMACY')  ? Promise.all([
                  ...
                  this.api.fhc().Addressbookcontroller().searchOrgUsingGET(this.api.keystoreId, this.api.tokenId, this.credentials.ehpassword, _.trim(recipientFilter) + '*', ( recipientType.id === 'NIHII-PHARMACY' ? "PHARMACY" : "HOSPITAL" ) ).then(searchResults=>_.chain(searchResults).filter(i=>!!_.get(i,'nihii',false) || !!_.get(i,'ssin',false) || !!_.get(i,'ehp',false) || !!_.get(i,'cbe',false)).uniqBy('ehp').value())
              ]) :
              // Default type = NIHII
              Promise.all([
          ...
          searchProm
              .then(searchResults => { if (reqIdx === this.recipientReqIdx) {
                  ...
                  if(!!_.get(this,"_assignRecipientValueWhenReplyingToMessage") && _.size(_.get(this, "filteredRecipientsList[0]",{})) && !!newMsgTo) {
                      ...
                      setTimeout(()=>{this._addRecipient()},100)
                      setTimeout(()=>{newMsgTo.close()},200)
                  }

              }})
              .catch(e=>{console.log("ERROR with _recipientsFilterChanged: ", e);})
              .finally(() => this.set('busySpinner', false))
...
<transaction>
  ...
  _saveMedexKhmerIttToPatFile() {
      ...
      return this.api.message().newInstanceWithPatient(_.get(this,"user",{}), _.get(this,"_data.currentPatient",{}))
          ...
          .catch(e => { console.log("[ERROR] _saveMedexKhmerIttToPatFile", e); return Promise.resolve(); })
  ...
  open(componentOpenParameters) {
      ...
      if(!!_.size(dataFromPatDetail)) {
          ...
          try{ delete(componentOpenParameters.dataFromPatDetail); } catch(e){}
          ...
          if(_.trim(_.get(dataFromPatDetail,"patient.id",""))) {
              ...
              setTimeout(()=>{ this.set("newMessage.document.title", _.compact([this.localize('documentTransfer', 'Document transfer', this.language), _.trim(_.get(this, "newMessage.document.title"))]).join(" - ")) },100)
          }
          ...
          return  this.api.document().getDocument(_.trim(_.get(dataFromPatDetail,"documentId","")))
              .then(foundDocument => this.api.crypto().extractKeysFromDelegationsForHcpHierarchy(_.get(this,"user.healthcarePartyId", null), _.trim(_.get(foundDocument,"id","")), _.size(_.get(foundDocument,"encryptionKeys",[])) ? _.get(foundDocument,"encryptionKeys",[]) : _.get(foundDocument,"delegations",[]))
                  .then(({extractedKeys: enckeys}) => !!_.trim(_.get(foundDocument,"attachmentId","")) ? this.api.document().getAttachment(_.trim(_.get(foundDocument,"id","")), _.trim(_.get(foundDocument,"attachmentId","")), enckeys.join(',')).then(decryptedContent=>({foundDocument,decryptedContent})).catch(e=>{console.log("ERROR with getAttachment: ",e); return foundDocument;}) : foundDocument)
              )
      ...
      if(!!_.size(componentOpenParameters)) {
          ...
          return prom
              ...
              .then(foundPatientBySsin => {
                  ...
                  linkedPatientComboxBox.selectedItem = _.assign({displayName : _.compact([
              ...
              .catch((e)=>{ console.log("ERROR with open: ", e); return prom; })
              .finally(()=>{
  ...
  _confirmMessageSuccessfullySent() {
      ...
      setTimeout(() => { this.$["success"].classList.add('displayNotification'); },100);
      setTimeout(() => { this.$["success"].classList.remove('displayNotification'); },8000);
  ...
  _prepareEMediattestMessage(dataFromPatDetail) {
      ...
      return promResolve
          ...
          .then(() => {
              ...
              if(linkedPatientComboxBox && !!_.trim(_.get(dataFromPatDetail, "patient.ssin", ""))) linkedPatientComboxBox.selectedItem = _.get(dataFromPatDetail, "patient", {})
              if(linkedPatientComboxBox && !_.trim(_.get(dataFromPatDetail, "patient.ssin", ""))) linkedPatientComboxBox.value = _.get(dataFromPatDetail, "patient.displayName", "")
              setTimeout(()=>{ this.set("newMessage.document.title", _.compact(["eMediAtt - Medex", _.trim(_.get(this, "newMessage.document.title"))]).join(" - ")) },100)
          })
          .then(() => outgoingDocumentComponent.printMedexPdfItt(dataFromPatDetail))
          .then(printMedexPdfIttData => this.set("_data",_.merge(printMedexPdfIttData, {language:this.language, nowMs: moment().format('YYYYMMDDHHmmss0000'), nowYYYYMMDD: moment().format('YYYY-MM-DD'), nowHHMMSS: moment().format('HH:mm:ss'),})))
          ...
          .then(([createdMessage,createdDocument]) => _.merge(this._data.eMediattest.khmerData, {createdMessage:createdMessage,createdDocument:createdDocument}))
          .then(() => {
          ...
          .then(hasError => {
              ...
              this.push('newMessage.recipients', { type:_.get(this,"medexData.recipient.type"), ehp:_.get(this,"medexData.recipient.id"), quality:_.get(this,"medexData.recipient.quality"), lastName: "Medex", firstName: "Medex"})
              ...
              // KHMER FILE
              const attachmentSizeKhmer = _.get(this.api.crypto().utils.text2ua(_.get(this,"_data.eMediattest.khmerContent","")),"byteLength",0)
              const attachmentSizeKhmerPow = attachmentSizeKhmer > (1024**2) ? 2 : attachmentSizeKhmer > 1024 ? 1 : 0
              this.push("_additionalAnnexesFromForward", {
                  content: this.api.crypto().utils.text2ua(_.get(this,"_data.eMediattest.khmerContent","")),
                  filename: _.trim(_.get(this, "_data.pdf.filename", "")).replace(".pdf",".xml"),
                  fileExtension: "xml",
          ...
          .catch( e => { console.log("ERROR _prepareEMediattestMessage", e); this.set("_bodyOverlay", true); this.$["unableToLinkPatDocument"].open(); })
          .finally(() => { this._resetLoadingMessage(); this.set('_isLoading', false ); })
  ...
  _addRecipient(){
      ...
      const recipientType = !!_.trim(_.get(newMsgTo,"selectedItem.cbe")) ? "CBE" :
          !!_.trim(_.get(newMsgTo,"selectedItem.ehp")) ? "EHP" :
          !!_.trim(_.get(newMsgTo,"selectedItem.ssin")) ? "SSIN" :
          "NIHII"
      ...
      _.map(keysToClean, k => !_.trim(_.get(newMsgTo.selectedItem, k, "")) ? false : newMsgTo.selectedItem[k] = newMsgTo.selectedItem[k].replace(/[^0-9]*/g,""))

      this.push('newMessage.recipients', _.merge(_.get(newMsgTo,"selectedItem",{}),{type:recipientType}))
  ...
  _wrapRecipientData(recipientObject) {
      ...
      const recipientId = !!_.trim( _.get(recipientObject, "cbe", "")) ? _.trim( _.get(recipientObject, "cbe", "")) :
      ...
      return {
          identifierType: { type:
              // _.trim(_.get(recipientObject,"type","")).toLowerCase() === "nihii-pharmacy" ? "PHARMACY" :
              // _.trim(_.get(recipientObject,"type","")).toLowerCase() === "nihii-hospital" ? "HOSPITAL" :
                  _.trim(_.get(recipientObject,"type",""))
          },
          id:recipientId,
          quality: !!_.trim(_.get(recipientObject,"quality","")) ? _.trim(_.get(recipientObject,"quality","doctor")).toUpperCase() : "DOCTOR",
          applicationId: _.trim(_.get(recipientObject,"applicationID","")),
          lastName: !!_.trim(_.get(recipientObject,"lastName","")) ? _.trim(_.get(recipientObject,"lastName","")) : recipientId,
          firstName: !!_.trim(_.get(recipientObject,"firstName","")) ? _.trim(_.get(recipientObject,"firstName","")) : recipientId,
          organizationName: (!_.trim(_.get(recipientObject,"lastName","")) && !_.trim(_.get(recipientObject,"firstName",""))) ? _.trim(_.get(recipientObject, "name", _.get(recipientObject, "companyName", ""))) : "",
  ...
  _doSendMessageWithEhBox(){
      ...
      this.api.hcparty().getCurrentHealthcareParty().then(currentHcp =>{
          ...
          const messageToBeSentWithEhbox = {
              ...
              destinations : _.map(_.get(this,"newMessage.recipients",[]), singleRecipient => this._wrapRecipientData(singleRecipient)),
              sender : {
