    ...
    return Polymer.html`
		...
		<template is="dom-if" if="[[!readOnly]]">
			<div class="relative">
				...
				<paper-fab class="button--icon-btn add-button" icon="add" on-tap="_add"></paper-fab>
			</template>
			<template is="dom-if" if="[[!_singleAction(layoutItem.editor.compulsoryFormGuids.*, layoutItem.editor.optionalFormGuids.*)]]">
					<paper-fab id="add" class="button--icon-btn add-button" icon="add" on-tap="_openAdd"></paper-fab>
		...
		<div class="subform-container">
			<template id="subforms-repeat" is="dom-repeat" items="[[subContexts]]" as="subContext">
				<div class="relative">
					...
					<dynamic-form id="sf-[[index]]" api="[[api]]" user="[[user]]" template="[[_getTemplate(subContext)]]" data-map="[[subContext.dataMap]]" data-provider="[[subContext.dataProvider]]" reports="[[formReports]]" read-only="[[readOnly]]" is-sub-form="" i18n="[[i18n]]" resources="[[resources]]" language="[[language]]" no-print="[[noPrint]]" parent-form-dp="[[parentFormDp]]"></dynamic-form>
				</div>
			</template>
		</div>
`;
