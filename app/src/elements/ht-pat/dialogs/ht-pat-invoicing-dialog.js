        ...
        <paper-dialog id="invoiceDialog" opened="{{opened}}">
            ...
            <div class="detail-panel">
                ...
                <div class="buttons">
                    ...
                    <paper-button class="button" on-tap="_closeDialog">[[localize('clo','close',language)]]</paper-button>
                    ...
                    <template is="dom-if" if="[[!isInvoiceClosed]]">
                        <template is="dom-if" if="[[hasInsurability]]"><paper-button class="button button--other" on-tap="_verifyTarification" disabled="[[_isETarButtonDisabled(selectedInvoice,isLoadingValidation,isTarificationMcnError,npi,selectedInvoice.sentMediumType)]]">[[localize('eTarif','Validation eTarif',language)]]</paper-button></template>
                        <template is="dom-if" if="[[!hasInsurability]]"><paper-button class="button button--other" on-tap="" disabled="true">[[localize('patient_has_no_insurance','Ce patient n\\'est pas assuré',language)]]</paper-button></template>
                    ...
                    <template is="dom-if" if="[[isInvoiceClosed]]">
                        <paper-button on-tap="_switchBetweenMethodForJustif" class="button" disabled="[[isLoadingJustif]]">[[localize('val_justificatory','Justificatory',language)]]</paper-button>
                    </template>
                </div>
        </paper-dialog>
...
`;
  ...
  _verifyTarification() {
      if (this.patient.ssin && this.api.tokenId && this.selectedInvoice.invoiceDate !== "" && this.api.patient().isValidSsin(this.patient.ssin)) {
          ...
          !this.api.patient().isValidSsin(this.patient.ssin) ? errors.push({code : "999", msgFr : "Niss patient invalide"}) : null
  ...
  _hasInsurability() { return ( this.patient && this.patient.insurabilities && !!this.patient.insurabilities.length ); }
  ...
  _filterThirdPartyJustitifications(inputData) {
      return _.filter( inputData, (v)=>{ return parseInt(v.id) !== 9; });
  }

  _totalChanged(typedValue){
      let v = Number(typedValue.detail.value || 0)
      if (isNaN(v)) {v = Number(this.activeNmclItem.reimbursement)+Number(this.activeNmclItem.patientIntervention)}
