                                ...
                                <div class="containScroll">
                                    <vaadin-grid id="noscroll" items="[[archiveListingMessages]]">
                                        ...
                                        <vaadin-grid-column flex-grow="0" width="20%" class="">
                                            ...
                                            <template><paper-button class="button button--other downloadListingArchive" on-tap="_downloadListingArchive" messageid="[[item.id]]"><iron-icon icon="icons:cloud-download" class="force-left"></iron-icon> &nbsp; [[localize('download','Télécharger',language)]]</paper-button></template>
                                        </vaadin-grid-column>
                                    </vaadin-grid>
                                </div>
                            </div>
...
`;
                  ...
                  .then(pl => {
                      pl.rows = _
                          ...
                          .filter((i)=>{return(
                              ...
                              && !!parseInt( _.size( _.filter( _.get(i, "medicalHouseContracts", [] ), (i)=>{return !!i && _.trim(_.get(i,"hcpId", "something")) === _.trim(_.get(this,"user.healthcarePartyId","else"))}) ) )
                          )})
                          .uniqBy( 'ssin' )
                          .value()
                          .map((i) => {
  ...
  _createOrUpdateMedicalHousePatientsInvoices() {
      ...
      // TOP-435
      const childrenInsurancesData = _.compact(_.uniqBy(_.map( this.flatRateInvoicingDataObject.insurancesData, i=>i), 'id'))
                  ...
                  prom = prom.then(pats =>
                      this.api.crypto().extractDelegationsSFKs(pat, this.user.healthcarePartyId)
                          .then(secretForeignKeys => {
                              ...
                              return retry.retry(() => (this.api.invoice().appendCodes(this.user.id, "patient", "cdrom", (pat.finalInsurability && pat.finalInsurability.insuranceId ? pat.finalInsurability.insuranceId : ''), secretForeignKeys.extractedKeys.join(","), null, (365*2), pat.invoicingCodes)))
                                  .then(invoices =>
                                      !_.trim(_.get(invoices, "[0].id", "")) ? this.api.invoice().newInstance(this.user, pat, invoices[0])
                                          ...
                                          .then(inv => {/*console.log(inv);*/ return inv}) : Promise.resolve(invoices[0]))
                                  .then(newInvoice => {
                                      //console.log("newInvoice", _.cloneDeep(newInvoice));
                                      if(this.invoiceHasDoubles(newInvoice)){
                                          //only remove the doubles
                                          pat.invoicingCodes.forEach(pic => {
                                            newInvoice.invoicingCodes = _.remove(newInvoice.invoicingCodes, ic => ic.dateCode === pic.dateCode && ic.code === pic.dateCode);
                                          });
                                          newInvoice.invoicingCodes = newInvoice.invoicingCodes.concat(pat.invoicingCodes);
                                          return this.api.invoice().modifyInvoice(newInvoice).then(inv =>this.api.register(inv,'invoice')).then(newInvoiceMod => {
                          ...
                          return _.compact(
                              _.map(invoicesToBeCorrected, i =>{
                                  return (
                                      ...
                                      && _.every(_.get(i,"invoicingCodes",[]), ic=>{ return _.get(ic,"pending",false)===true && _.get(ic,"resent",false)===true && _.get(ic,"lost",false)===false })
                                  ) ? i : false
                              })
                          )
                      })
  ...
  _exportFlatRateInvoicing() {
      ...
      this.api.hcparty().getHealthcareParty(_.get(this.user, "healthcarePartyId", "" ))
          ...
          .then(pendingInvoicesToResend => {
              ...
              //("pending invoices to resend", _.cloneDeep(pendingInvoicesToResend));
          ...
          .then(patsAndPatsToResend=>{
              ...
              const commonPatientsBetweenExportsWithSameIns = _.compact(
                  _.map(myPatients, pat => {
                      const matchingPat = _.filter(myPatientsToResend, patToResend => { return (
              ...
              const nonCommonPatientsToAdded = _.compact( _.map( _.map(myPatientsToResend, "id"), patId => {
                  if( resolvedPatientsToBeResent.indexOf(patId) > -1 ) { return false; } else { resolvedPatientsToBeResent.push(patId); }
  ...
  // _runFlatRateCheck(){
  //     return this._flatrateCheckOnAllPatients(201909).then(resList =>{
  //         console.log("resList", resList);
  //         this.set("flatRateCheckList", resList);
  //         return resList;
  //     });
  // }
  //
  ...
  //     //return this._getPatientsByHcp(_.get(this.hcp, "id")).then(myPatients => Promise.all(myPatients.map(pat =>  flatRateUtil.checkFlatrateData(pat, invoicedMonth))));
  //     return this._getPatientsByHcp(_.get(this.hcp, "id")).then(myPatients => myPatients.map(pat =>  flatRateUtil.checkFlatrateDataWithHcp(this.hcp, pat, invoicedMonth)));
  // }


  //data display methods
  _patDataSelection(resList, list){
