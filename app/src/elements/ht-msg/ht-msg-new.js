...
`;
  ...
  _loadRecipientTypesTranslations() {
    ...
    _loadOrganizationQualitiesTranslations() {
      ...
      this.set("organizationQualities", _.orderBy(_.compact(_.map(_.get(this,"resources."+this.language,{}), (v,k) => !!_.trim(k).toLowerCase().startsWith("ehboxquality_organization_") ? {id:_.trim(k).replace(/ehboxquality_organization_/ig, ""),label:v, checked:(defaultCheckedQualities.indexOf(_.trim(k).replace(/ehboxquality_organization_/ig, "")) > -1)} : false)), ["label"],["asc"]))
    }
...
<transaction>
  ...
  _doSendMessageWithEhBox(){
      ...
      _addRecipientEhBox(e) {
          ...
          try { ehboxToAdd = JSON.parse(_.get(_.filter(_.get(e,"path",[]), nodePath=> !!_.trim(_.get(nodePath,"dataset.ehbox",""))),"[0].dataset.ehbox","")); } catch(e) {}
          const finalRecipientObject = _.assign(_.cloneDeep(_.get(this,"_recipientWithMultipleEHealthBoxes",{})), {ehealthBoxes:[_.find(_.get(this,"_recipientWithMultipleEHealthBoxes.ehealthBoxes",[]), {id:_.trim(_.get(ehboxToAdd,"id","")), quality:_.trim(_.get(ehboxToAdd,"quality",""))})]})
