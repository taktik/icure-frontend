  ...
  static get template() {
    return html`
		...
		<paper-dialog id="dialog" opened="{{opened}}" style="overflow: auto">
					...
					<div class="layout vertical center">
						...
						<vaadin-grid id="currentEhKeychainList" items="[[CurrentEhKeychains]]">
							...
							<vaadin-grid-column flex-grow="4">
								...
								<template>
									<paper-button on-tap="_SetMainKeystore" keystore\$="[[index]]"><iron-icon icon="[[getActiveButtonIconName(item.onlyDecrypt)]]" keystore\$="[[index]]"></iron-icon></paper-button>
								</template>
							</vaadin-grid-column>
							<vaadin-grid-column flex-grow="4">
								<template class="header">
								...
								<template>
									<paper-button on-tap="_SetMHKeystore" keystore\$="[[index]]"><iron-icon icon="[[getActiveMMButtonIconName(item.personalKeystore)]]" keystore\$="[[index]]"></iron-icon></paper-button>
								</template>
							</vaadin-grid-column>
							<vaadin-grid-column flex-grow="4">
								<template class="header">
								...
								<template>
									<paper-button on-tap="_openKeystorePasswordDialog" index\$="[[index]]"><iron-icon icon="[[getPasswordButtonIconName(item.hasPassword)]]" index\$="[[index]]"></iron-icon></paper-button>
								</template>
							</vaadin-grid-column>
                            <vaadin-grid-column flex-grow="1">
                                ...
                                <template>
                                    <div class="cell frozen">
                                        <paper-button hidden="[[_displayAltDeleteButton(item)]]"><iron-icon icon="delete" on-tap="_removeKeystore" keystore\$="[[item.keystore.name]]"></iron-icon></paper-button>
            ...
            <div class="buttons">
				<paper-button class="modal-button modal-button--save" on-tap="_openEhealthKeystorePage">[[localize('ask_new_keystore','Demander la création de votre keystore, le renouveler ou le revoquer',language)]]</paper-button>
                <paper-button dialog-confirm="" autofocus="" class="modal-button modal-button--save">[[localize('done','Done',language)]]</paper-button>
            </div>
		</paper-dialog>
		<paper-dialog id="keystorePasswordDialog">
			...
			<div class="buttons">
				...
				<paper-button autofocus="" on-tap="_setPassword" class="button button--save"><iron-icon icon="save"></iron-icon>[[localize('save','Save',language)]]</paper-button>
			</div>
		</paper-dialog>
`;
  }
  ...
  _openEhealthKeystorePage() {
      var link = document.createElement("a")
      document.body.appendChild(link)
      link.style = "display: none"
      link.target = "_blank"
      if(this.language == "nl") {
          link.href = "https://www.ehealth.fgov.be/nl/egezondheid/beroepsbeoefenaars-in-de-gezondheidszorg/beheer-van-de-ehealth-certificaten/algemene-voorstelling"
      } else {
          link.href = "https://www.ehealth.fgov.be/fr/esante/professionnels-de-la-sante/gestion-des-certificats-ehealth"
      }
      link.click()
	}
